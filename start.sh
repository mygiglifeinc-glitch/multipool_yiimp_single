...
# PHP version to install (from /etc/multipool.conf; older conf files lack it).
...
echo
if [[ "$UsingDomain" == "yes" && "$InstallSSL" == "yes" ]]; then
	echo -e "You can access your admin panel at,$BLUE https://${DomainName}/site/${AdminPanel} $COL_RESET"
else
	echo -e "You can access your admin panel at,$BLUE https://${DomainName}/site/${AdminPanel} $COL_RESET"
